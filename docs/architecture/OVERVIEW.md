...
| File | Scope |
|------|-------|
| [`01-routing-and-bandit-scoring.md`](01-routing-and-bandit-scoring.md) | Thompson-sampling bandit router, reliability posteriors, factor weights (reliability/speed/intelligence/headroom/rate-limit), 10% explore floor, fallback loop with per-model exhaustion diagnostics |
| [`02-quota-and-cooldown-engine.md`](02-quota-and-cooldown-engine.md) | RPM/RPD/TPM/TPD accounting, concurrency leases, cooldown ladder (90s → 2m → 10m → 1h → 1d), probe-based early recovery, back-off from error body/header (#798), provider-quota pooling (openrouter::free, google::project) |
